...
class BitField extends ClassAnnotation {
  ...
  void apply(Class target, LibraryBuilder output) {
    ...
    Class cs = Class((b) => b
      ...
      ..fields.addAll(
          target.fields.where((b) => b.resolvedAnnotationsOfType<Bits>().length == 1).map((e) {
      ...
      ..fields.addAll(
          target.fields.where((b) => b.resolvedAnnotationsOfType<Bits>().length == 1).map((e) => Field((b) => b
            ..name = '${e.name}Mask'
            ..type = refer('int')
            ..modifier = FieldModifier.constant
            ..assignment = Code('((0x01 << ${e.name}Bits) - 1) << ${e.name}Offset')
            ..static = true
          ))
      )
      ..methods.addAll(
          target.fields.where((b) => b.resolvedAnnotationsOfType<Bits>().length == 1).map((e) => Method((b) => b
            ..name = e.name
            ..type = MethodType.getter
            ..returns = refer('int')
            ..lambda = true
      ...
      ..methods.addAll(
          target.fields.where((b) => b.resolvedAnnotationsOfType<Bits>().length == 1).map((e) => Method((b) => b
            ...
            ..lambda = true
            ..annotations.add(refer('override'))
            ..body = Code('value = (value & ~${e.name}Mask) | ((v << ${e.name}Offset) & ${e.name}Mask)')
          ))
      )
    );
    output.body.add(cs);
